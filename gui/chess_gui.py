"""
Chess Engine GUI — Flask API backend.
...
# Catch-all: serve React app (production) or hint to use Vite (dev)
    ...
    try:
        for pair_idx, movetime in enumerate(movetimes):
            ...
            for game_in_pair in range(1, 3):
                ...
                # Don't record if game was aborted mid-way
...
def get_eval_state_route():
    ...
    # Build SAN move list for the current game
    san_moves = []
    tmp = chess.Board()
